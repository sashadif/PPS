...
ParallelSimulationModel model = null;
        try {
            model = getModelCore(2,tmodel, 20, 10, 3); // n and k/n
...
//        modelA.printStatistics();
    ...
    public static PetriObjModel getSimpleModel(int numThreads, int complexity) throws ExceptionInvalidNetStructure, ExceptionInvalidTimeDelay {
    ...
    public static ParallelSimulationModel getModel(double tsim, int numThreads, int threadComplexity, int maxBuffer) throws ExceptionInvalidNetStructure, ExceptionInvalidTimeDelay {
        ArrayList<PetriSim> list = new ArrayList<>();
    ...
    public static ParallelSimulationModel getModelCore(int cores, double tsim, int numThreads, int threadComplexity, int maxBuffer) throws ExceptionInvalidNetStructure, ExceptionInvalidTimeDelay {
        ArrayList<PetriSim> list = new ArrayList<>();
...
//        ParallelSimulationModel model = getModel(tmodel, 2, 100, 3);
          ParallelSimulationModel model = null;        
        try {
            model = getModelCore(cores,tmodel, numThreads, threadComplexity, limit);
        } catch (ExceptionInvalidTimeDelay ex) {
            Logger.getLogger(Model.class.getName()).log(Level.SEVERE, null, ex);
        }
