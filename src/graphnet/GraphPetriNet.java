...
public class GraphPetriNet implements Cloneable, Serializable {
    ...
    public GraphPetriNet(GraphPetriNet graphPetriNet) {
        ...
        bulkCopyNoPasteElements(elementsToCopy, graphPetriNet.graphArcInList, graphPetriNet.graphArcOutList);
        ...
        for (GraphArcOut arcOut : arcOutSource) {
            for (GraphArcIn arcIn : arcInSource) {
                ...
                if (inBeginId == outEndId && inEndId == outBeginId) {
                     if (!arcOut.isFirstArc() && !arcOut.isSecondArc()) { //july 2023 to provide the oposite GraphArcOut does not exist
    ...
    public void paintGraphPetriNet(Graphics2D g2, Graphics g) { // edited 26.07.2018 by Inna
     ...
     // передаються чомусь неправильні значенння перша чи друга дуга у restoreSavedState(), 
     // цей фрагмент трохи виправляє ситуацію і малює правильно при першому натисканні на панель
         for (GraphArcOut arcOut : graphArcOutList) { // 8 august 2023
            for (GraphArcIn arcIn : graphArcInList) {
                int inBeginId = ((GraphPetriPlace)arcIn.getBeginElement()).getId();
                int inEndId = ((GraphPetriTransition)arcIn.getEndElement()).getId();
                int outBeginId = ((GraphPetriTransition)arcOut.getBeginElement()).getId();
                int outEndId = ((GraphPetriPlace)arcOut.getEndElement()).getId(); // буває Exception тут при якійсь дії малювання
                if (inBeginId == outEndId && inEndId == outBeginId) {
                     if (!arcOut.isFirstArc() && !arcOut.isSecondArc()) { //july 2023 to provide the oposite GraphArcOut does not exist
                        arcIn.twoArcs(arcOut);                        
                        break;
